...
- Locked economics (Pro copy):  
  ...
  - Haven takes 0% of materials, 0% of tips, and 0% of Inspection Visits. 100% of approved materials and 100% of tips go to the Pro.  
  - Pro receives: labor payout + approved materials + tips + any applicable diagnosis/service fee.  
...
- Standard (non‑diagnosis) materials‑decline outcome — approved and implemented in the Pro App: label it “Job Ended — Materials Declined” (distinct from the diagnosis path). A flat $30 Convenience Fee applies for that outcome. Existing diagnosis fees and the `inspection_completed` path remain as-is. Customer-side alignment is pending; no enum/code changes in this PR.
- Labor economics — current, founder‑confirmed:\n+  - Current working margin is 20% of the customer labor/service price. Explicitly adjustable later. Not a permanent immutable rule. Not TBD.\n+  - Haven keeps 20%. Pro receives 80%.\n+  - The Pro sees that exact payout before accepting and receives exactly that amount.\n+  - Haven's margin is inside the customer's listed labor price. It is not a deduction after acceptance.\n+  - Haven takes 0% from materials, 0% from tips, and 0% from diagnosis / Inspection Visit fees.
...
- **Superseded by v0.9** — §7.2 is resolved in the Pro App: standard-category materials decline now applies a flat $30 Convenience Fee.
...
- **Resolved (Pro App)** — Standard (non-diagnosis) jobs that get materials declined mid-`in_progress` now pay the pro a flat $30 Convenience Fee. Customer App alignment remains a follow-up.
...
Terminology note: For standard (non‑diagnosis) jobs where materials are declined mid‑`in_progress`, the approved product label is “Job Ended — Materials Declined” with a flat $30 Convenience Fee (implemented in Pro). Diagnosis jobs continue to use `inspection_completed`. Do not introduce a new canonical enum in this PR; alignment across apps is DEFERRED.
...
| `platformFee` | number | ❌ not present | ❌ not implemented in app code yet — the Pro UI shows a fixed payout with no fee modeling | Founder decision (current working model): Haven keeps 20% inside the listed customer labor price; the Pro’s fixed labor payout is 80% of that listed labor price. The Pro sees that exact payout before accepting and receives exactly that amount. The fee is not a post-accept deduction. Adjustable later, not TBD. |
...
2. **Resolved in Pro; Customer alignment pending:** for standard (non‑diagnosis) jobs, the materials‑declined path ends as “Job Ended — Materials Declined” with a flat $30 Convenience Fee (implemented in Pro). Diagnosis jobs remain unchanged (`inspection_completed` with the $45 Inspection Visit). Do not change enums or code in this PR; align naming across apps later.
